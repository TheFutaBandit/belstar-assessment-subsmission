...
const TaskList = () => {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        console.log(`filterComplete - ${completeFilter}, filterIncomplete - ${incompleteFilter}`)
        ...
        if((completeFilter || incompleteFilter) > 0) {
          const filterType = (completeFilter) ? "complete" : "incomplete";
      
          console.log(`The Filter type is ${filterType}`);
      
          finalTasks = finalTasks.filter(item => item.status === filterType);
        }

        if(debounce) {
          finalTasks = finalTasks.filter((item) => item.title.includes(debounce))
        }
  ...
  }, [completeFilter, incompleteFilter, debounce]);
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(debounceTimeId);
  }, [query])

  // useEffect(() => {
    
  //   if(debounce) {
  //     setFilteredTasks((prev) => prev.filter((item) => item.title.includes(debounce)));
  //   } else {
  ...
  const filterTask = (filterFlag) => {
    ...
    // const filterType = (completeFilter) ? "complete" : "incomplete";
    const filterType = filterFlag;

    console.log(`The Filter type is ${filterType}`);

    const newFilteredTasks = tasks.filter(item => item.status === filterType);
  ...
  const toggleCompleteFilter = () => {
    if(incompleteFilter === true) setIncompleteFilter(!incompleteFilter);
    setCompleteFilter(!completeFilter);
  }

  const toggleIncompleteFilter = () => {
    if(completeFilter === true) setCompleteFilter(!completeFilter);
    ...
    <div className="bg-white p-4 rounded-lg shadow max-h-96 overflow-y-auto">
      <div className = "flex justify-between">
        ...
        <div className = "filters flex gap-2">
          <p 
            className={"text-sm font-semibold mb-4 border-b pb-2 cursor-pointer" + (completeFilter ? " text-green-600" : " text-gray-800")}
            onClick = {() => {
          ...
          <p 
            className={"text-sm font-semibold mb-4 text-gray-800 border-b pb-2 cursor-pointer" + (incompleteFilter ? " text-red-600" : " text-gray-800")}
            onClick = {() => {
      ...
      <div class = "search-bar flex justify-between gap-2">
        <Search color='black' />
        <input 
          className = "border grow mb-3 text-black px-2 text-sm h-full" 
          type = "text" 
